...
/// the invoked subcommand, so `vp format` stays `format` here, and `None` means
/// the user asked for something else that ran a built-in on their behalf.
pub(super) fn print(command: Option<&str>, cwd: &AbsolutePath) {
    let Some(command) = command else { return };
    // A task spawned this command, so the user is already on the `vpr` path the
